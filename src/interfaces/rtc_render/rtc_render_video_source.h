...
class RTCRenderVideoTrackSource : public rtc::AdaptedVideoTrackSource {
 public:
  RTCRenderVideoTrackSource()
    : rtc::AdaptedVideoTrackSource(), _is_screencast(false) {}

  RTCRenderVideoTrackSource(const bool is_screencast, const absl::optional<bool> needs_denoising)
    : rtc::AdaptedVideoTrackSource(), _is_screencast(is_screencast), _needs_denoising(needs_denoising) {}

  ~RTCRenderVideoTrackSource() override {
...
class RTCRenderVideoSource
  : public Napi::ObjectWrap<RTCRenderVideoSource> {
 public:
  explicit RTCRenderVideoSource(const Napi::CallbackInfo&);
